...
public class TurbulenceZipfTraffic extends Turbulence{
	...
	protected final double exp;
		...
		public TurbulenceZipfTraffic(String prefix) {
			super(prefix);
		...
		public boolean add() {
		    ...
		    Message lookupMessage = generateTopicLookupMessage(topicString);
		    newKad.getNode().setTopic(topicString, newNode);

		    if(registerMessage != null) EDSimulator.add(time, registerMessage, newNode, newNode.getKademliaProtocol().getProtocolID());
		    //start lookup messages later
		    if(lookupMessage != null)EDSimulator.add(2*KademliaCommonConfig.AD_LIFE_TIME + time, lookupMessage, newNode, newNode.getKademliaProtocol().getProtocolID());
