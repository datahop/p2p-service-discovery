...
public class Discv5TopicTable { // implements TopicTable {
    ...
    protected long getWaitingTime(TopicRegistration reg, long curr_time) {
        ...
        if (topicQ != null && topicQ.size() == this.adsPerQueue) {
            ...
            waiting_time = 0;
        	//if(topicQ != null&&topicQ.size()>0)waiting_time = 1000*topicQ.size();
        	//else waiting_time = 0;
    ...
    protected Ticket [] makeRegisterDecision(long curr_time) {   
        ...
        for (Topic topic : topicSet) {
        	...
        	//System.out.println("Ticketlist "+topic.getTopic()+" "+tickets.size());
    ...
    protected Ticket getTicket(Topic t, KademliaNode advertiser, long rtt_delay, long curr_time) {
        ...
        if (waiting_time == -1) {
            //already registered
            KademliaObserver.reportWaitingTime(topic, waiting_time,Util.logDistance(topic.getTopicID(),hostID));
